...
object Query2 {
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder()
      .appName("Project1-Query2-Handshakes")
      .getOrCreate()
  ...
  val bcGrid = sc.broadcast(gridMap)
    ...
    // Parse People
    val peopleRDD2 = peopleRaw.filter(!_.contains("id")).map(line => {
    val parts = line.split(",")
    // Add all 6 fields
    (parts(0), parts(1).toDouble, parts(2).toDouble, parts(3), parts(4).toDouble, parts(5))
    })

    // Get all distinct IDs from the first query's results
    // .map(_._1) gets the first ID in the (ID1, ID2) pair
    val uniquePairs = resultRDD.map(pair => pair._1).distinct().collect().toSet

    // Broadcast the set of unique IDs to all executors
    val bcWhitelist = sc.broadcast(uniquePairs)

    // Filter peopleRDD2 to only include those in the whitelist
    val connectedPeople = peopleRDD2.filter(person => {
    val whitelist = bcWhitelist.value
    whitelist.contains(person._1)
    })

    val totalCount = connectedPeople.count()
    println(s"Query 2 complete. Found $totalCount unique people who had a handshake.")

    // Show sample
    connectedPeople.take(10).foreach(println)
